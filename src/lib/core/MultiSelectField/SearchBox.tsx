...
export enum SortType { Default, Name }
...
> {
  ...
  render() {
    ...
    const numChecked = options.map(option => isElementSelected(value, option)).filter(c => c).length;
    const checked = options.length > 0 && numChecked === options.length;
    const indeterminate = options.length > 0 && numChecked > 0 && numChecked < options.length;
    ...
    const sortActionOptions = (
      <>
        <ActionBar.SelectOption
          label='Default'
          icon={isDefaultSortModeRevert ? <SwapVertReverseIcon /> : <SwapVertIcon />}
          selected={isDefaultSortMode}
          onClick={() => {
            const newSortMode = {
              type: SortType.Default,
              isReverseSort: isDefaultSortMode ? !sortMode.isReverseSort : false,
            };
        ...
        />
        <ActionBar.SelectOption
          icon={isNameSortModeRevert ? <SwapVertReverseIcon /> : <SwapVertIcon />}
          label='Name'
    ...
    const sortAction = (
      <ActionBar.Select
          ...
          open={isSortMode}
          onOpen={() => this.setState(({isSortMode: prevIsSortMode}) => ({isSortMode: !prevIsSortMode}))}
          disabled={disabled}
        >
        {sortActionOptions}
      </ActionBar.Select>
